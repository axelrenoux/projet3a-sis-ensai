...
*/
	...
	throws SAXException{
		if(qName.equals("id")){
			...
			inAlbum = true;
			if(premierAlbum) listeAlbums = new HashMap<String, Album>();
			premierAlbum = false;
...
throws SAXException{
	if(qName.equals("id")){
		...
		//on met a jour l'artiste de la chanson
		System.out.println("avant mise a jour " + chanson.getArtiste().getUrl());
		chanson.getArtiste().setUrl(currentArtisteChanson.getUrl());
		System.out.println("apres mise a jour " + chanson.getArtiste().getUrl());
	}else if(qName.equals("url") && inArtisteChanson){
		inURLArtisteChanson = false;
	}else if(qName.equals("album")){
		inAlbum = false;
		listeAlbums.put(rangCurrentAlbum, currentAlbum);
		chanson.setAlbums(listeAlbums);
	}else if(qName.equals("artist") && inAlbum){
		inArtisteAlbum = false;
		currentAlbum.setArtiste(currentArtisteAlbum);//TODO
	}else if(qName.equals("title") && inAlbum){
		inTitleAlbum = false;
	}else if(qName.equals("url") && inAlbum){
		inURLAlbum = false;	
